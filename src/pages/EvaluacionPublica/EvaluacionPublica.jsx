// src/pages/EvaluacionPublica/EvaluacionPublica.jsx  — v6
// FIX #7: variables correctas (no intento_id/habilidad_id como undefined),
//         usa pregunta_id (no id), usa contenido (no texto).
import { useState, useEffect } from 'react'
import { useSearchParams } from 'react-router-dom'
import { evaluacionService } from '../../services'
import styles from './EvaluacionPublica.module.css'

function BarraProgreso({ paso, max = 8 }) {
  const pct = Math.min(100, Math.round((paso / max) * 100))
  return (
    <div style={{ marginBottom: 16 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between',
        fontSize: '.73rem', color: 'var(--text-muted)', marginBottom: 6 }}>
        <span>Pregunta {paso} de ~{max}</span>
        <span>{pct}%</span>
      </div>
...
export default function EvaluacionPublica() {
  ...
  const [pregunta, setPregunta]   = useState(null)       // objeto con pregunta_id, contenido, opciones
  ...
  const [seleccionado, setSeleccionado] = useState(null)  // id de respuesta seleccionada
  ...
  const cargarAcceso = async () => {
    try {
      ...
      // El backend devuelve: intento_id, compania_id, habilidad_id, pregunta, evaluacion_descripcion
      setIntentoId(d.intento_id)
      setHabilidadId(d.habilidad_id)
      setEvalDesc(d.evaluacion_descripcion || 'Evaluación de Competencias')
      setPregunta(d.pregunta)              // { pregunta_id, contenido, opciones, numero }
  ...
  const responder = async () => {
    ...
    try {
      const res = await evaluacionService.responder({
        ...
        pregunta_id:  pregunta.pregunta_id, // campo correcto del objeto pregunta
    ...
    return (
      <div className={styles.page}>
        <div className={styles.card}>
          ...
          {resultado && (
            <div style={{ background: 'var(--primary-bg)', borderRadius: 12,
      ...
      <div className={styles.card}>
        ...
        {theta != null && (
          <div style={{ fontSize: '.72rem', color: 'var(--text-muted)',
            ...
            θ actual: {Number(theta).toFixed(3)}
          </div>
        )}
        ...
        {error && (
          <div style={{ background: 'var(--danger-bg)', color: 'var(--danger)',
            ...
            fontSize: '.85rem', display: 'flex', justifyContent: 'space-between' }}>
            {error}
        ...
        {/* Enunciado — usa contenido (campo correcto del modelo) */}
        <p style={{ fontSize: '1rem', fontWeight: 600, color: 'var(--text-h)',
          lineHeight: 1.6, marginBottom: 20 }}>
          {pregunta?.contenido}
        </p>

        {/* Opciones — usa op.contenido (campo correcto) */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 24 }}>
          {pregunta?.opciones?.map(op => (
            <button key={op.id} onClick={() => setSeleccionado(op.id)}
              style={{
                padding: '14px 18px', borderRadius: 12, cursor: 'pointer',
                textAlign: 'left', fontFamily: 'var(--font)', fontSize: '.9rem',
                ...
                border: `2px solid ${seleccionado === op.id
        ...
        {/* Confirmar */}
        <button onClick={responder} disabled={!seleccionado || loading}
          style={{
            ...
            border: 'none',
            cursor: seleccionado && !loading ? 'pointer' : 'not-allowed',
